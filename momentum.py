...
def momentum(df, candles, ticker, interval):
    print("Inside Momentum")
    
    mom_values = abstract.MOM(df, candles).to_frame()
    mom_values.dropna(how='all', inplace=True)
    mom_values.rename(columns={mom_values.columns[0]: 'momentum'}, inplace=True)
    mom_values['is_hot'] = mom_values['momentum'] > None
    mom_values['is_cold'] = mom_values['momentum'] < None
    print(mom_values)
    return mom_values
